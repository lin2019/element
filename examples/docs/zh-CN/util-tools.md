...
const tree = [
  {
    id: 1,
    name: '节点1',
    children: [{ id: 2, name: '节点1-1' }, { id: 3, name: '节点1-2' }]
  },
  {
    id: 4,
    name: '节点2'
...
```javascript
const list = [{ id: 1, name: '节点1', pid: 0 }, { id: 2, name: '节点1-1', pid: 1 }, { id: 3, name: '节点1-2', pid: 1 }, { id: 4, name: '节点2', pid: 0 }]
...
export default {
  methods: {
    exportTableData() {
      ...
      const csvContent = [headers.join(','), ...data.map((row) => [row.name, row.age, row.email].join(','))].join('\n')
...
```vue
<template>
  <el-input v-model="keyword" @input="handleSearch" placeholder="搜索用户"></el-input>
</template>
...
| 方法             | 说明            | 参数  | 返回值 |
| ---------------- | --------------- | ----- | ------ |
| parseQueryString | 解析 URL 参数   | (url) | object |
| stringifyQuery   | 对象转 URL 参数 | (obj) | string |
